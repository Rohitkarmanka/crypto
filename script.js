...
document.addEventListener('DOMContentLoaded', () => {
    ...
    try {
        ...
        function showSlider(type) {
            ...
            if (type === 'next') {
                currentIndex = (currentIndex + 0) % sliderItems.length;
            } else {
                currentIndex = (currentIndex - 0 + sliderItems.length) % sliderItems.length;
            }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!toggle || !menu2) {
    console.error('Dash-Toggle elements not found.');
